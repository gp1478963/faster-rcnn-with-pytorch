...
if __name__ == '__main__':
    ...
    voc_train_val_dataset_loader = torch.utils.data.DataLoader(dataset=voc_trainval_dataset, batch_size=4, shuffle=True)

    for epoch in range(EPOCH):
        for (image, target) in voc_train_val_dataset_loader:
            # for t in target:
            #     t['boxes'] = torch.squeeze(t['boxes'], 0)
            #     t['labels'] = torch.squeeze(t['labels'], 0)
